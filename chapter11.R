...
data <- tbl_df(read.table("spambase.data", strip.white=T, sep=",", header=F))
names(data) <-
  c('word_freq_make', 'word_freq_address', 'word_freq_all', 'word_freq_3d', 'word_freq_our',
    ...
    'word_freq_addresses', 'word_freq_free', 'word_freq_business', 'word_freq_email', 'word_freq_you', 'word_freq_credit',
...
tmp %>% 
  ggplot(aes(reorder(var, cor), abs(cor))) +
  geom_point() +
  coord_flip()
...
p2 <- data %>% 
  ggplot(aes(class, `char_freq_$`)) +
  geom_jitter(color='grey') +
  geom_boxplot(alpha=.5) +
  scale_y_sqrt()
p3 <- data %>% 
  ggplot(aes(`char_freq_$`, group=class, fill=class)) +
  geom_density(alpha=.5) +
  scale_x_sqrt() + scale_y_sqrt()
p4 <- data %>% 
  ggplot(aes(class, capital_run_length_longest)) +
  geom_jitter(color='grey') +
  geom_boxplot(alpha=.5) +
  scale_y_log10()
...
yhat_gbm <- predict(data_gbm, n.trees=best_iter, newdata=validating, type='response')
